...
@Test func briefDeliveryPolicySkipsWhenAlreadyDelivered() {
    ...
    let main = SleepSession(start: at(calendar, day: 15, hour: 22, minute: 0), end: at(calendar, day: 16, hour: 5, minute: 0))
...
@Test func briefDeliveryPolicyDedupeChecksMainSleepEndNotNow() {
    ...
    let main = SleepSession(start: at(calendar, day: 16, hour: 21, minute: 0), end: mainSleepEnd)
...
@Test func briefDeliveryPolicyDeliversPartialSummaryPastCutoffEvenWithoutMetrics() {
    let calendar = utcCalendar()
    let main = SleepSession(start: at(calendar, day: 15, hour: 22, minute: 0), end: at(calendar, day: 16, hour: 5, minute: 0))
    let now = at(calendar, day: 16, hour: 10, minute: 0)
...
@Test func briefDeliveryPolicyEarlyWakeWaitsUntilSixThirty() {
    let calendar = utcCalendar()
    let main = SleepSession(start: at(calendar, day: 15, hour: 21, minute: 0), end: at(calendar, day: 16, hour: 5, minute: 0)) // wakes at 5:00
...
@Test func briefDeliveryPolicyLateWakeNeverDeliversAfterTen() {
    ...
    let main = SleepSession(start: at(calendar, day: 15, hour: 22, minute: 0), end: at(calendar, day: 16, hour: 9, minute: 45))
