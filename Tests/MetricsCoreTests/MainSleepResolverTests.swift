...
@Test func mainSleepResolverPicksNightOverAfternoonNap() {
    let nap = SleepSession(start: t("2026-07-16T14:00:00Z"), end: t("2026-07-16T15:30:00Z")) // 1.5h nap
    let night = SleepSession(start: t("2026-07-15T22:00:00Z"), end: t("2026-07-16T06:00:00Z")) // 8h
...
@Test func mainSleepResolverPicksLongerOfTwoSimilarSessions() {
    let sessionA = SleepSession(start: t("2026-07-15T22:00:00Z"), end: t("2026-07-16T05:30:00Z")) // 7.5h
    let sessionB = SleepSession(start: t("2026-07-16T06:00:00Z"), end: t("2026-07-16T13:45:00Z")) // 7.75h
...
@Test func mainSleepResolverRejectsSessionShorterThanTwoHours() {
    let shortSession = SleepSession(start: t("2026-07-16T04:00:00Z"), end: t("2026-07-16T05:30:00Z")) // 1.5h
...
@Test func mainSleepResolverReturnsNilWhenCandidateEndedTenMinutesAgo() {
    ...
    let stillAsleepCandidate = SleepSession(start: t("2026-07-15T22:00:00Z"), end: t("2026-07-16T05:50:00Z")) // ended 10 min ago
...
@Test func mainSleepResolverExcludesSessionsEndingOutsideWindow() {
    let now = t("2026-07-16T06:00:00Z")
    let tooEarly = SleepSession(start: t("2026-07-14T22:00:00Z"), end: t("2026-07-15T06:00:00Z"))
    let window = DateInterval(start: t("2026-07-15T18:00:00Z"), end: now)
...
@Test func mainSleepResolverNeverTreatsHistoricalSessionAsStillAsleepRegardlessOfWindowEndProximity() {
    ...
    let session = SleepSession(start: t("2026-07-12T22:00:00Z"), end: historicalSessionEnd)
...
@Test func mainSleepResolverHandlesDaylightSavingFallBackWithinWindow() {
    ...
    #expect(main == session)
    #expect(main?.duration == TimeInterval(9 * 60 * 60))
}
